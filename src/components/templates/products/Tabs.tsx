...
const Tabs = ({ product }: ProductTypes) => {
  ...
  return (
    <div className="flex flex-col bg-white w-full  mt-6 ">
      {/* Head Titles */}
      ...
      >
        ...
        <p className="text-xs sm:text-sm md:text-base font-light leading-8 text-pretty ">
          قهوه همواره فراتر از یک نوشیدنی ساده بوده و بخشی از سبک زندگی افراد در
          سراسر جهان به شمار می‌رود. این محصول با هدف ارائه تجربه‌ای مطلوب از
          عطر و طعم قهوه تهیه شده و می‌تواند همراه مناسبی برای شروع روز، جلسات
          کاری یا لحظات استراحت باشد.
        </p>
        <p className="text-xs sm:text-sm md:text-base font-light leading-8 text-pretty">
          دانه‌های قهوه مورد استفاده در این محصول با دقت انتخاب شده‌اند تا
          تعادلی مناسب میان تلخی، شیرینی طبیعی و رایحه دلنشین ایجاد شود. فرآیند
          برشته‌کاری کنترل‌شده به حفظ ویژگی‌های اصلی دانه کمک کرده و طعمی
          یکنواخت و باکیفیت را در هر فنجان تضمین می‌کند.
        </p>
        <p className="text-xs sm:text-sm md:text-base font-light leading-8 text-pretty">
      ...
      {/* Attribute Section */}
      <div
        className={`${tabAttribute ? "flex flex-col gap-5 px-6 pb-8 items-start my-5 border-b border-b-gray-100" : "hidden"}`}
        dir="rtl"
      >
