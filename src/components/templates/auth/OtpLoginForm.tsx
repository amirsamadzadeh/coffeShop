...
import { otpLoginAction } from "@/actions/auth";
...
export const OtpLoginForm = ({ setFormStatus }: Props) => {
  const [state, formAction, isPending] = useActionState(otpLoginAction, {
    success: false,
    message: "",
  });
  useEffect(() => {
    state.success && setFormStatus("OTP_VERIFY");
  }, [state.success, setFormStatus]);
  return (
    <div className="flex flex-col mb-6 animate-fadeIn">
      ...
      <form className="flex flex-col px-4 gap-2" dir="rtl" action={formAction}>
        ...
        >
          {isPending ? "درحال ارسال کد" : " ارسال کد یکبار مصرف"}
        </button>
      </form>
    </div>
  );
